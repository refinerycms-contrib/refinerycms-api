module Refinery
  module Api
    module V1
      class OptionValuesController < Refinery::Api::BaseController
        def index
          if params[:ids]
        ...
        def create
          authorize! :create, Refinery::OptionValue
        ...
        private
          ...
          def scope
            if params[:option_type_id]
              @scope ||= Refinery::OptionType.find(params[:option_type_id]).option_values.accessible_by(current_ability, :read)
            else
              @scope ||= Refinery::OptionValue.accessible_by(current_ability, :read).load
            end
          end
