require 'spec_helper'
require 'refinery/testing_support/bar_ability'
...
module Refinery
  describe Api::V1::OrdersController, :type => :controller do
    ...
    let(:current_api_user) do
      user = Refinery.user_class.new(:email => "refinery@example.com")
    ...
    context "the current api user is not persisted" do
      let(:current_api_user) { Refinery.user_class.new }
    ...
    describe 'current' do
      ...
      context "multiple incomplete orders exist" do
        it "returns the latest incomplete order" do
          new_order = Refinery::Order.create! user: order.user
    ...
    it "can not view someone else's order" do
      allow_any_instance_of(Order).to receive_messages :user => stub_model(Refinery::LegacyUser)
      api_get :show, :id => order.to_param
      assert_unauthorized!
    end
    ...
    it "can view an order if the token is passed in header" do
      request.headers["X-Refinery-Order-Token"] = order.guest_token
      api_get :show, :id => order.to_param
      expect(response.status).to eq(200)
    end

    context "with BarAbility registered" do
      before { Refinery::Ability.register_ability(::BarAbility) }
      after { Refinery::Ability.remove_ability(::BarAbility) }

      it "can view an order" do
        user = mock_model(Refinery::LegacyUser)
        allow(user).to receive_message_chain(:refinery_roles, :pluck).and_return(["bar"])
        allow(user).to receive(:has_refinery_role?).with('bar').and_return(true)
        allow(user).to receive(:has_refinery_role?).with('admin').and_return(false)
        allow(Refinery.user_class).to receive_messages find_by: user
        api_get :show, :id => order.to_param
        expect(response.status).to eq(200)
      end
    end
    ...
    it "assigns email when creating a new order" do
      api_post :create, :order => { :email => "guest@refinerycommerce.com" }
      expect(json_response['email']).not_to eq controller.current_api_user
      expect(json_response['email']).to eq "guest@refinerycommerce.com"
    end

    # Regression test for #3404
    it "can specify additional parameters for a line item" do
      expect(Order).to receive(:create!).and_return(order = Refinery::Order.new)
    ...
    context "admin user imports order" do
      before do
        allow(current_api_user).to receive_messages has_refinery_role?: true
        allow(current_api_user).to receive_message_chain :refinery_roles, pluck: ["admin"]
      end
    ...
    context "working with an order" do
      ...
      context "line_items hash not present in request" do
        it "responds successfully" do
          api_put :update, :id => order.to_param, :order => {
            :email => "hublock@refinerycommerce.com"
          }
      ...
      it "cannot set the user_id for the order" do
        user = Refinery.user_class.create
    ...
    context "as an admin" do
      ...
      context "with no orders" do
        before { Refinery::Order.delete_all }
      ...
      context "search" do
        before do
          create(:order)
          Refinery::Order.last.update_attribute(:email, 'refinery@refinerycommerce.com')
        end

        let(:expected_result) { Refinery::Order.last }

        it "can query the results through a parameter" do
          api_get :index, :q => { :email_cont => 'refinery' }
          expect(json_response["orders"].count).to eq(1)
          expect(json_response["orders"].first).to have_attributes(attributes)
