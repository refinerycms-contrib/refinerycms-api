...
module Refinery
  describe Api::V1::CheckoutsController, type: :controller do
      ...
      context 'ensure no double_render_error' do
        before do
          def controller.load_order(*)
            respond_with(@order, default_template: 'refinery/api/v1/orders/show', status: 200)
          end
        end
    ...
    before(:each) do
      stub_authentication!
      Refinery::Config[:track_inventory_levels] = false
    ...
    after do
      Refinery::Config[:track_inventory_levels] = true
    end
    ...
    context "PUT 'update'" do
      ...
      context "transitioning to delivery" do
        ...
        it "can update addresses but not transition to delivery w/o shipping setup" do
          Refinery::ShippingMethod.destroy_all
          api_put :update,
            id: order.to_param, order_token: order.guest_token,
            order: {
              bill_address_attributes: address,
          ...
          expect(json_response['error']).to eq(I18n.t(:could_not_transition, scope: "refinery.api.order"))
          expect(response.status).to eq(422)
        end
      ...
      it "returns errors when source is missing attributes" do
        ...
        api_put :update, id: order.to_param, order_token: order.guest_token,
          ...
          payment_source: {
            @payment_method.id.to_s => { name: "Refinery" }
          }
      ...
      it "can transition from confirm to complete" do
        ...
        allow_any_instance_of(Refinery::Order).to receive_messages(payment_required?: false)
        api_put :update, id: order.to_param, order_token: order.guest_token
      ...
      context "as an admin" do
        ...
        it "can assign a user to the order" do
          ...
          api_put :update, id: order.to_param, order_token: order.guest_token,
            order: { user_id: user.id, email: "guest@refinerycommerce.com" }
          expect(response.status).to eq(200)
      ...
      it "can assign an email to the order" do
        api_put :update, id: order.to_param, order_token: order.guest_token,
          order: { email: "guest@refinerycommerce.com" }
    ...
    context "PUT 'next'" do
      ...
      it "cannot transition to address without a line item" do
        ...
        order.update_column(:email, "refinery@example.com")
        api_put :next, id: order.to_param, order_token: order.guest_token
        expect(response.status).to eq(422)
        expect(json_response["errors"]["base"]).to include(Refinery.t(:there_are_no_items_for_this_order))
      end
      ...
      it "doesnt advance payment state if order has no payment" do
        ...
        expect(json_response["errors"]["base"]).to include(Refinery.t(:no_payment_found))
      end
    ...
    context "PUT 'advance'" do
      ...
      it 'continues to advance advances an order while it can move forward' do
        expect_any_instance_of(Refinery::Order).to receive(:next).exactly(3).times.and_return(true, true, false)
