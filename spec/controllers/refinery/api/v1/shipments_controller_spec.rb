...
describe Refinery::Api::V1::ShipmentsController, :type => :controller do
  render_views
  ...
  context "as an admin" do
    ...
    it "can make a shipment ready" do
      allow_any_instance_of(Refinery::Order).to receive_messages(:paid? => true, :complete? => true)
      api_put :ready
      expect(json_response).to have_attributes(attributes)
    ...
    it "cannot make a shipment ready if the order is unpaid" do
      allow_any_instance_of(Refinery::Order).to receive_messages(:paid? => false)
      api_put :ready
    ...
    context "can transition a shipment from ready to ship" do
      before do
        ...
        allow_any_instance_of(Refinery::ShippingRate).to receive_messages(:cost => 5)
      end
